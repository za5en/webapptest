...
export function useTelegram() {
    ...
    if (tg.initDataUnsafe !== null) {
      ...
      if (typeof tg.initDataUnsafe.user !== 'undefined' && tg.initDataUnsafe.user !== null) {
        var userInfo = JSON.parse(JSON.stringify(tg.initDataUnsafe.user));
        userInfo.first_name = encodeURIComponent(userInfo.first_name)
        userInfo.username = encodeURIComponent(userInfo.username)
        userInfo.last_name = encodeURIComponent(userInfo.last_name)

        tokenString += `user=${JSON.stringify(userInfo)}&`
      }
      if (typeof tg.initDataUnsafe.receiver !== 'undefined' && tg.initDataUnsafe.receiver !== null) {
        var receiverInfo = JSON.parse(JSON.stringify(tg.initDataUnsafe.receiver));
        receiverInfo.first_name = encodeURIComponent(receiverInfo.first_name)
        receiverInfo.username = encodeURIComponent(receiverInfo.username)
        receiverInfo.last_name = encodeURIComponent(receiverInfo.last_name)

        tokenString += `receiver=${JSON.stringify(receiverInfo)}&`
      }
      if (typeof tg.initDataUnsafe.chat !== 'undefined' && tg.initDataUnsafe.chat !== null) {
        var chatInfo = JSON.parse(JSON.stringify(tg.initDataUnsafe.chat));
        chatInfo.title = encodeURIComponent(chatInfo.title)
        chatInfo.username = encodeURIComponent(chatInfo.username)

        tokenString += `chat=${JSON.stringify(chatInfo)}&`
      }
